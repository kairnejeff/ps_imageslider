...
class Ps_ImageSlider extends Module implements WidgetInterface
{
    ...
    protected function _postValidation()
    {
        ...
        if (Tools::isSubmit('submitSlider')) {
            ...
            foreach ($languages as $language) {
                ...
                if (Tools::getValue('mobile_image_' . $language['id_lang']) != null && !Validate::isFileName(Tools::getValue('image_' . $language['id_lang']))) {
                    $errors[] = $this->getTranslator()->trans('Invalid filename.', array(), 'Modules.Imageslider.Admin');
                }
                if (Tools::getValue('mobile_image_old_' . $language['id_lang']) != null && !Validate::isFileName(Tools::getValue('image_old_' . $language['id_lang']))) {
                    $errors[] = $this->getTranslator()->trans('Invalid filename.', array(), 'Modules.Imageslider.Admin');
                }
            }
    ...
    protected function _postProcess()
    {
        ...
        if (Tools::isSubmit('submitSlider')) {
            ...
            foreach ($languages as $language) {
                ...
                $type2 = Tools::strtolower(Tools::substr(strrchr($_FILES['mobile_image_'.$language['id_lang']]['name'], '.'), 1));
                $imagesize = @getimagesize($_FILES['image_'.$language['id_lang']]['tmp_name']);
                $mobile_imagesize = @getimagesize($_FILES['mobile_image_'.$language['id_lang']]['tmp_name']);
                if (isset($_FILES['image_'.$language['id_lang']]) &&
                    ...
                    if ($error = ImageManager::validateUpload($_FILES['image_'.$language['id_lang']])) {
                        $errors[] = $error;
                    } elseif ($error = ImageManager::validateUpload($_FILES['mobile_image_'.$language['id_lang']])){
                        $errors[] = $error;
                    }elseif (!$temp_name || !move_uploaded_file($_FILES['image_'.$language['id_lang']]['tmp_name'], $temp_name)) {
                        return false;
                    } elseif (!$temp_name2 || !move_uploaded_file($_FILES['mobile_image_'.$language['id_lang']]['tmp_name'], $temp_name2)) {
                        return false;
                    ...
                    }elseif (!ImageManager::resize($temp_name2, dirname(__FILE__).'/images/'.$salt.'_'.$_FILES['mobile_image_'.$language['id_lang']]['name'], null, null, $type2)) {
                        $errors[] = $this->displayError($this->getTranslator()->trans('An error occurred during the image upload process.', array(), 'Admin.Notifications.Error'));
                    }elseif (!$this->generate_webp_image($temp_name, __DIR__.'/images/'.$salt.'_'.$_FILES['image_'.$language['id_lang']]['name'].'.webp',100)){
                        $errors[] = $this->displayError($this->getTranslator()->trans('Can not convertir this image en webp.', array(), 'Admin.Notifications.Error'));
                    }elseif (!$this->generate_webp_image($temp_name2, __DIR__.'/images/'.$salt.'_'.$_FILES['mobile_image_'.$language['id_lang']]['name'].'.webp',100)){
                        $errors[] = $this->displayError($this->getTranslator()->trans('Can not convertir this image en webp.', array(), 'Admin.Notifications.Error'));
                    }
                    if (isset($temp_name)) {
                        @unlink($temp_name);
    ...
    public function getSlides($active = null)
    {
        ...
        foreach ($slides as &$slide) {
            ...
            $slide['image_webp_url'] ='modules/ps_imageslider/images/'.$slide['image'].'.webp';
            $slide['mobile_image_url'] = $this->context->link->getMediaLink(_MODULE_DIR_.'ps_imageslider/images/'.$slide['mobile_image']);
            $slide['mobile_image_webp_url'] ='modules/ps_imageslider/images/'.$slide['mobile_image'].'.webp';
        ...
        $fields_form = array(
            'form' => array(
                ...
                'input' => array(
                    ...
                    array(
                        'type' => 'file_lang',
                        'label' => $this->getTranslator()->trans('Mobile Image', array(), 'Admin.Global'),
                        'name' => 'mobile_image',
                        'required' => true,
                        'lang' => true,
                        'desc' => $this->getTranslator()->trans('Maximum image size: %s.', array(ini_get('upload_max_filesize')), 'Admin.Global')
